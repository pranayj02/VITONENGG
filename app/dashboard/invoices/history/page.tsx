...
import { Search, X, ExternalLink, Receipt } from "lucide-react";
  ...
  return (
    <div className="p-6 lg:p-8 max-w-5xl mx-auto">
      <div className="flex items-center justify-between mb-6 gap-4">
        ...
        <Link href="/dashboard/invoices/new" className="flex items-center gap-2 bg-viton-red hover:bg-viton-red-hover dark:bg-orange-500 dark:hover:bg-orange-600 text-white text-sm font-semibold px-4 py-2.5 rounded-xl transition-all">
          <Receipt size={16} />
          New Invoice
        </Link>
      </div>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="bg-[#f1f3f8] dark:bg-gray-800/50 border-b border-[#dde1ea] dark:border-gray-800">
                  {["Invoice No.", "Buyer", "Date", "Amount", "Status", ""].map((h) => (
                ...
                {filtered.map((row, i) => (
                  <tr key={row.id} className={`border-b border-[#dde1ea] dark:border-gray-800 last:border-0 hover:bg-[#f7f8fb] dark:hover:bg-gray-800/30 transition-colors ${i % 2 === 0 ? "" : "bg-[#fafbfd] dark:bg-transparent"}`}>
                    <td className="px-5 py-4">
                      <span className="font-mono text-viton-red dark:text-orange-400 font-semibold text-xs">{row.invoice_number}</span>
                    </td>
                    ...
                    <td className="px-5 py-4">
                      <Link href={`/dashboard/invoices/${row.id}`} className="text-[#8892a8] dark:text-gray-500 hover:text-viton-red dark:hover:text-orange-400 transition-colors">
