        ...
        <div className="space-y-4">
          ...
          <div className="sm:hidden space-y-3">
            {filtered.map((req) => {
              const isRejecting = rejectingId === req.id;
              return (
                <div key={req.id} className="bg-white dark:bg-gray-900 rounded-2xl shadow-sm border border-[#dde1ea] dark:border-gray-800 overflow-hidden">
                  <div className="p-4">
                    <div className="flex items-center justify-between gap-4">
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-viton-navy dark:text-white text-sm font-bold mt-1 leading-tight">{req.item_name}</p>
                        <p className="text-[#8892a8] dark:text-gray-500 text-xs mt-1">
                          {req.item_unit} · {req.qty} {req.adjustment_type === "adjustment_in" ? "Add" : "Remove"}
                        </p>
                        {req.notes && <p className="text-[#8892a8] dark:text-gray-500 text-xs mt-0.5 line-clamp-2">{req.notes}</p>}
                      </div>
                    ...
                    {req.status === "approved" && req.reviewed_by_name && (
                      <p className="text-xs text-green-600 dark:text-green-400 mt-2">✓ Reviewed by {req.reviewed_by_name}</p>
                    )}
                    {req.status === "rejected" && req.reviewed_by_name && (
                      <div className="mt-2">
                        <p className="text-xs text-red-500">✗ Rejected by {req.reviewed_by_name}</p>
                        {req.review_note && <p className="text-xs text-red-500 mt-0.5">"{req.review_note}"</p>}
                      </div>
                    )}
                    {req.status === "pending" && (
                      <div className="flex gap-2 pt-3 mt-3 border-t border-[#eef1f6] dark:border-gray-800/50">
                        <button onClick={() => handleApprove(req)} disabled={processingId === req.id}
                          className="flex-1 min-h-[48px] flex items-center justify-center gap-2 bg-green-500 hover:bg-green-600 text-white text-sm font-semibold px-4 py-2.5 rounded-xl transition-all">
                        ...
                        <button onClick={() => { setRejectingId(isRejecting ? null : req.id); setRejectNote(""); }} disabled={processingId === req.id}
                          className={`flex-1 min-h-[48px] flex items-center justify-center gap-2 text-white text-sm font-semibold px-4 py-2.5 rounded-xl transition-all ${
