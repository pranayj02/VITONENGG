...
export default function EditWOPage() {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      const w = wo as unknown as WorkOrder & { items: WorkOrderItem[] };
      setWoNumber(w.wo_number);
      setOriginalWoNumber(w.wo_number);
      setPartyName(w.party_name || "");
      setDeliveryDate(w.delivery_date || "");
      setPoNo(w.po_no || "");
      setPoDate(w.po_date || "");
      setInspectionBy(w.inspection_by || "NO");
      setQapNo(w.qap_no || "");
      setItems(w.items?.length ? w.items : [{ ...EMPTY_ITEM(), sr_no: 1 }]);
      setLoading(false);
    }
    load();
  }, [woId]);

  // Check WO number uniqueness when it changes
  useEffect(() => {
    if (!woNumber.trim() || woNumber === originalWoNumber) {
      setWoExists(false);
      return;
    }
  ...
  }, [woNumber, originalWoNumber, woId]);
  ...
  async function handleSave() {
    ...
    if (!partyName.trim()) { setError("Customer is required."); return; }
    ...
    try {
      ...
      await audit({ action: "updated", entity_type: "work_order", entity_id: woId as string, entity_code: woNumber.trim() });
      router.push("/dashboard/wo");
    } catch (e: any) {
      ...
      <div className="bg-white dark:bg-gray-900 border border-[#dde1ea] dark:border-gray-800 rounded-xl p-5 mb-4">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4">
          {[
            ...
            { label: "Customer", value: partyName, setter: setPartyName, placeholder: "INDIA GLYCOLS LTD - GORAKHPUR", required: true, wide: true },
            ...
            <div key={f.label} className={(f as any).wide ? "col-span-2 sm:col-span-3 lg:col-span-2" : ""}>
              ...
              <input
                ...
                className={`w-full bg-[#f6f8fc] dark:bg-gray-950 border border-[#dde1ea] dark:border-gray-700 rounded-lg px-3 py-2 text-sm text-viton-navy dark:text-white placeholder:text-[#8892a8] dark:placeholder:text-gray-600 focus:outline-none focus:ring-2 focus:ring-viton-red/20 dark:focus:ring-orange-500/20 focus:border-viton-red dark:focus:border-orange-500 transition-all ${
                  f.label === "WO Number" && woExists ? "border-red-300 dark:border-red-500/40 focus:border-red-500" : ""
                }`}
              />
              {(f as any).formatHint && (
                <p className="text-[10px] text-[#8892a8] dark:text-gray-500 mt-0.5">Format: {(f as any).formatHint}</p>
              )}
              {f.label === "WO Number" && woExists && (
